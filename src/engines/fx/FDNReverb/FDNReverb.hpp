...
struct FDNReverb : Td {
  ...
  FDNReverb& resize(std::initializer_list<unsigned> combDelays,
...
namespace otto::engines {
  ...
  struct KratosReverb : EffectEngine<KratosReverb> {
    static constexpr std::string_view name = "Kratos";
    struct Props {
      Property<float> filter = {0, has_limits::init(0, 1), steppable::init(0.01)};
      Property<float> shimmer = {0, has_limits::init(0, 1), steppable::init(0.01)};
      Property<float> length = {0.5, has_limits::init(0, 1), steppable::init(0.01)};
      Property<float> damping = {0.4, has_limits::init(0, 0.99), steppable::init(0.01)};

...
Tv SimplePitchShift<Tv>::operator()(Tv in) noexcept {
  ...
  for (int i=0; i<2; i++) {
    delay_amount[i] -= 1;
    if (delay_amount[i]<0) delay_amount[i] = 0;
...
namespace otto::engines {
            ...
            : EffectEngine("Kratos", props, std::make_unique<KratosReverbScreen>(this))
    {
      ...
      props.filter.on_change().connect([this](float flt) {
          pre_filter.freq(3000 + flt*flt*17000);
      });
      props.shimmer.on_change().connect([this](float sh) {
          shimmer_amount = sh * 0.03;
      });
      props.length.on_change().connect([this](float len) {
          reverb.decay(3.f * len);
      });
      props.damping.on_change().connect([this](float damp) {
          reverb.damping( damp );
      });

...
process = (multi_osc(osc1_freq)*(only_noise) , multi_osc(osc2_freq)*(only_noise) ) <: mixer :> VCF : *(envelope) : _ ;

midigate	= button ("/h:midi/gate");                             	// MIDI keyon-keyoff
midifreq	= hslider("/h:midi/freq[unit:Hz]", 440, 20, 1000, 1); 	// MIDI keyon key
midigain	= hslider("/h:midi/gain", 0.5, 0, 10, 0.01);	       	// MIDI keyon velocity
...
envelope = en.adsre(a,d,s,r,midigate)*(midigain);
	a = hslider("/h:envelope/Attack", 0.001, 0.001, 4, 0.001);
	d = hslider("/h:envelope/Decay", 0, 0, 4, 0.01);
	s = hslider("/h:envelope/Sustain", 1, 0, 1, 0.1) * 100;
	r = hslider("/h:envelope/Release", 0, 0, 4, 0.01);

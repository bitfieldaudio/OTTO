...
namespace otto::engines {
  ...
  bool OTTOFMSynthScreen::keypress(Key key)
  {
      ...
      case ui::Key::blue_click:
        engine.props.cur_op = 0;
      case ui::Key::green_click:
        engine.props.cur_op = 1;
      case ui::Key::white_click:
        engine.props.cur_op = 2;
      case ui::Key::red_click:
        engine.props.cur_op = 3;
  ...
  void OTTOFMSynthScreen::draw(ui::vg::Canvas& ctx)
  {
    ...
    ctx.fillStyle(Colours::Yellow);
    ctx.textAlign(HorizontalAlign::Right, VerticalAlign::Middle);
    ctx.fillText(fmt::format("{:1.2}", engine.props.operators.at(engine.props.cur_op).ratio_idx), {width - x_pad, y_pad + 2 * space});
    ...
    ctx.fillStyle(Colours::Red);
    ctx.textAlign(HorizontalAlign::Right, VerticalAlign::Middle);
    ctx.fillText(fmt::format("{:1.2}", engine.props.operators.at(engine.props.cur_op).mAtt), {width - x_pad, y_pad + 3 * space});
  }

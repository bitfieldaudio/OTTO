...
namespace otto::engines {
  ...
  struct TsarSynthScreen : EngineScreen<TsarSynth> {
    void draw(Canvas& ctx) override;
    bool keypress(Key key) override;
    void rotary(RotaryEvent e) override;

    using EngineScreen<TsarSynth>::EngineScreen;
  };

  // TsarSynth ////////////////////////////////////////////////////////////////

  TsarSynth::TsarSynth()
    : SynthEngine("Tsar", props, std::make_unique<TsarSynthScreen>(this)),
      voice_mgr_(props),
      faust_(std::make_unique<FAUSTCLASS>(), props)
  {}

  audio::ProcessData<1> TsarSynth::process(audio::ProcessData<1> data)
  {
  ...
  bool TsarSynthScreen::keypress(Key key)
  {
    return false;
  }

  void TsarSynthScreen::rotary(RotaryEvent e)
  {
    switch (e.rotary) {
  ...
  void TsarSynthScreen::draw(Canvas& ctx)
  {
    // dots
    ctx.group([&] {
      ctx.beginPath();

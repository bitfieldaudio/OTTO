...
namespace otto::engines {
  ...
  void SynthSampleScreen::rotary(ui::RotaryEvent e) {
    switch (e.rotary) {
    case ui::Rotary::Blue:
      engine->props.in.step(e.clicks); break;
    case ui::Rotary::Green:
      engine->props.out.step(e.clicks); break;
    case ui::Rotary::White:
      engine->props.speed.step(e.clicks); break;
    case ui::Rotary::Red:
      engine->props.mode.step(e.clicks); break;
    }
  }

  SynthSampleScreen::SynthSampleScreen(SynthSampler *m) :
    ui::EngineScreen<SynthSampler> (m),
    topWF (new util::audio::Waveform(engine->sampleData.size() / ui::vg::topWFsize.w / 4.0, 1.0)
           ),
    topWFW (topWF, ui::vg::topWFsize),
    mainWF (new util::audio::Waveform(50, 1.0)),
    mainWFW (mainWF, ui::vg::mainWFsize) {}

  void engines::SynthSampleScreen::draw(ui::vg::Canvas &ctx) {

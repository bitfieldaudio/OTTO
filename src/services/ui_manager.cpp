...
namespace otto::services {
  ...
  UIManager::UIManager()
  {
    state.current_screen.observe_no_immediate_call(this, [&](auto new_val) {
    ...
    state.active_channel.observe_no_immediate_call(this, [this, old_chan = state.active_channel.get()](auto new_chan) mutable {
      if (new_chan == old_chan) return;
      old_chan = new_chan;
      state.current_screen = state.current_screen.get();
    });

    state.octave.observe_no_immediate_call(this, [&](auto octave) {
      LEDColor c = [&] {

...
namespace otto::services {
  ...
  struct DebugUI {
    ...
    std::vector<std::function<void()>> draw_funcs;

    template<typename Callable>
    inline std::enable_if_t<util::is_invocable_v<Callable>, std::size_t> add_info(
      Callable&& callable)
    {
    ...
    template<typename Callable>
    inline std::enable_if_t<util::is_invocable_v<Callable>> add_info(Callable&& callable)
    {}
#endif
  };

#if OTTO_DEBUG_UI
  inline DebugUI::Info::Info()
  {
    index = add_info(*this);
  }

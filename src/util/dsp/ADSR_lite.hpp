...
public:
    ...
    Env_lite& loop(bool v){ mLoop=v; return *this; }

    /// Sets the point at which the envelope holds its value until released
    Env_lite& sustainPoint(int v){ mSustain=v; return *this; }

    /// Disable sustain
    Env_lite& sustainDisable(){ return sustainPoint(N); }
    ...
    Env_lite& lengths(Tp a, Tp b){ Tp v[]={a,b}; return lengths(v,2); }

    /// Set first three segment lengths
    Env_lite& lengths(Tp a, Tp b, Tp c){ Tp v[]={a,b,c}; return lengths(v,3); }

    /// Set first four segment lengths
    Env_lite& lengths(Tp a, Tp b, Tp c, Tp d){ Tp v[]={a,b,c,d}; return lengths(v,4); }

    /// Set first five segment lengths
    Env_lite& lengths(Tp a, Tp b, Tp c, Tp d, Tp e){ Tp v[]={a,b,c,d,e}; return lengths(v,5); }
    ...
    Env_lite& levels(Tv a, Tv b){ Tv v[]={a,b}; return levels(v,2); }

    /// Set first three break-point levels
    Env_lite& levels(Tv a, Tv b, Tv c){ Tv v[]={a,b,c}; return levels(v,3); }

    /// Set first four break-point levels
    Env_lite& levels(Tv a, Tv b, Tv c, Tv d){ Tv v[]={a,b,c,d}; return levels(v,4); }

    /// Set first five break-point levels
    Env_lite& levels(Tv a, Tv b, Tv c, Tv d, Tv e){ Tv v[]={a,b,c,d,e}; return levels(v,5); }
...
Env_lite<N,Tv,Tp,Td>::Env_lite(float (*voice)(void))
...
public:
    ...
    /// Set attack length
    ADSR_lite& attack(Tp len){ return setLen(0,len); }

    /// Set decay length
    ADSR_lite& decay(Tp len){ return setLen(1,len); }
    ...
    /// Set release length
    ADSR_lite& release(Tp len){ return setLen(2,len); }

    /// Set amplitude
    ADSR_lite& amp(Tv v){ this->maxLevel(v); return *this; }

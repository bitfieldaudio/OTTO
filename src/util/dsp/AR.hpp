...
public:
    ...
    /// Set attack length
    AR& attack(Tp len){ return setLen(0,len); }

    /// Set release length
    AR& release(Tp len){ return setLen(1,len); }

    /// Set amplitude
    AR& amp(Tv v){ this->maxLevel(v); return *this; }
    ...
    /// Get release length
    Tp release() const { return this->lengths()[1]; }

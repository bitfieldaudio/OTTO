...
namespace otto::core::midi {
  ...
  struct MidiEvent {
    ...
    enum class Type { NoteOff = 0b1000, NoteOn = 0b1001, ControlChange = 0b1011, PitchBend = 0b1110 };
  ...
  struct NoteEvent : MidiEvent {
    ...
    constexpr NoteEvent(MidiEvent::Type type, std::string_view note, float velocity = 1, byte channel = 0)
    ...
    /// Gives velocity as a float [0, 1]
    float fvelocity() const noexcept
    {
  ...
  using AnyMidiEvent = std::variant<MidiEvent, NoteOnEvent, NoteOffEvent, ControlChangeEvent, PitchBendEvent>;

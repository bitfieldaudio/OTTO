...
namespace otto::core::engine {
  ...
  EngineSelectorScreen::EngineSelectorScreen(IEngineDispatcher& ed)
    : engine_wid(engine_names,
                 eng_opts([&ed](int idx) -> IEngine& { return ed.select((ed.allow_off) ? idx - 1 : idx); })),
      ...
      _engine_dispatcher(ed)
  {
    ...
    util::transform(ed.make_name_list(), std::back_inserter(engine_names), [](auto& sv) { return std::string(sv); });
  }


  SelectorWidget::Options EngineSelectorScreen::eng_opts(std::function<IEngine&(int)>&& select_eg) noexcept
  {
  ...
  SelectorWidget::Options EngineSelectorScreen::prst_opts(std::function<IEngine&()>&& cur_eg) noexcept
  {
    SelectorWidget::Options opts;

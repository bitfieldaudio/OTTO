...
namespace otto::engines {
    ...
    std::pair<bool, int> get_sequencer_number(int key) {
        ...
        std::array<std::pair<bool, int>, 12> octave = {{{true, 0}, {false, 0}, {true, 1}, {false, 1},
                                                        {true, 2}, {true, 3}, {false, 2}, {true, 4},
                                                        {false, 3}, {true, 5}, {false, 4}, {true,6}}};
        bool is_white = octave[octave_pos].first;
        if (is_white) return {true, octave[octave_pos].second + (key / 12)*7};
        else return {false, octave[octave_pos].second + (key / 12)*5};
    }
            ...
            util::match(event,
                        [&](midi::NoteOnEvent& ev) {
                            ...
                            if (key.first) {
                                ...
                                channels[current_channel].triggers[step] = !channels[current_channel].triggers[step];
                            } else {

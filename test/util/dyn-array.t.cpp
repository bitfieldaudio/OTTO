...
SCENARIO("DynArrays can be resized")
{
  GIVEN("A DynArray of ints, with initial size 0")
  {
    ...
    WHEN("It is resized to a greater number")
    {
      ...
      {
        REQUIRE(std::all_of(std::begin(a), std::end(a), [](auto&& a) { return a == 0; }));
      }
    }
...
SCENARIO("DynArrays can be random-accessed")
{
  GIVEN("A DynArray of floats with a size greater than 0")
  {
    ...
    otto::util::dyn_array<float> a(size);
    CAPTURE(size);
  ...
  GIVEN("A DynArray of floats with a size greater than 0")
  {
    ...
    WHEN("All values are initialized with random values")
    {
      for (int i = 0; i < size; i++) {
        a[i] = Random::get(std::numeric_limits<float>::min(), std::numeric_limits<float>::max());
      }
    }
